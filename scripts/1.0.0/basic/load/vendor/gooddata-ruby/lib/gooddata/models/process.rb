  ...
  class Process < Rest::Resource
    ...
    class << self
      ...
      def deploy(path, options = { :client => GoodData.client, :project => GoodData.project })
        ...
        if path.to_s.start_with?('appstore://')
          brick_name = path.gsub('appstore://', '')

          fail 'You can not specify both - branch and tag.' if options[:branch] && options[:tag]
          ...
          Dir.mktmpdir do |dir|
            ...
            Dir.chdir(File.join(dir, 'app_store')) do
              ref = options[:branch] || options[:tag]
              ...
              brick_path = File.join(dir, 'app_store', 'apps', brick_name)

              unless File.exists?(brick_path)
                fail "Invalid brickname specified - '#{brick_name}'"
              end

              return deploy(brick_path, opts)
            end
          end
        end
